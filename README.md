...
- **ドラッグ＆ドロップで画像追加** - 画像ファイルをウィンドウにドラッグ＆ドロップして直接追加・差し替え
- **オンボーディング（使い方ガイド）** - 初回起動時にアプリの基本操作を案内するステップバイステップガイドを表示
- **調整パネルの数値入力・モニター選択** - 位置（X/Y）・サイズ（幅/高さ）の数値入力フィールドと、表示先モニターの選択ドロップダウンを搭載
- **メニューバーからの画像差し替え** - 「表示中: X体」サブメニューから各ウィンドウの画像を直接差し替え可能
- **レイアウトプリセット** - 現在のウィンドウ配置をプリセットとして保存・読み込み。シーンや気分に合わせてワンクリックで切り替え
- **フローティングメニュー** - ダブルクリックでフローティングメニューを表示。アイコンとテキストラベル付きのボタンで、切り取り・反転・調整・背景削除・表示をリセット・閉じるにワンクリックアクセス
- **画像の切り取り（トリミング）** - iPhone写真アプリ風のクロップエディタ。アスペクト比プリセット（フリー・オリジナル・1:1・3:2・4:3・16:9等）、90度回転、傾き補正、パースペクティブ補正（垂直・水平）、左右反転、Undo/Redo対応。元の画像を保持した非破壊編集
...
- 画像ファイル（PNG/JPEG/GIF/TIFF/HEIC）をウィンドウにドラッグ＆ドロップすると、新しいウィンドウとして追加されます
- Option キーを押しながらドロップすると、ドロップ先のウィンドウの画像を差し替えます
...
- 右クリック →「表示の調整」→「表示の調整...」で、回転と透明度の調整パネルを開きます
  ...
  - 位置・サイズ: X/Y座標と幅/高さを数値で直接入力可能
  - モニター選択: 表示先のモニターをドロップダウンから選択
...
- ダブルクリックでフローティングメニューが表示されます
- アイコンの下にテキストラベルが表示され、各ボタンの機能がひと目でわかります
- 切り取り・反転・調整・背景削除・表示をリセット・閉じるの操作にワンクリックでアクセスできます

**画像を切り取る（トリミング）**

- フローティングメニューの「切り取り」、または右クリック →「その他」→「切り取り」でクロップエディタが開きます
- iPhone写真アプリ風のエディタで、以下の編集が可能です：
  - **クロップ枠の調整**: 四隅と辺のハンドルをドラッグして表示領域を調整。画像のパン・ズームにも対応
  - **アスペクト比プリセット**: フリー・オリジナル・1:1・3:2・4:3・16:9 などから選択
  - **90度回転**: ボタンで画像を90度ずつ回転
  - **傾き補正**: ルーラーダイヤルで -45°〜+45° の範囲で水平傾きを補正
  - **パースペクティブ補正**: 垂直・水平のパースペクティブ（遠近感）を補正
  - **左右反転**: クロップエディタ内で画像を左右反転
  - **Undo/Redo**: 編集操作の取り消し・やり直し
- トップバーの「✓」で確定、「×」でキャンセル、「戻す」で全編集をリセット
- ESC キーでもキャンセルできます
- 切り取りは非破壊的で、元の画像はそのまま保持されます
...
現在のウィンドウ配置（画像・位置・サイズ・回転・透明度）をプリセットとして保存し、いつでもワンクリックで復元できます。

- メニューバーのアイコン →「レイアウト」→「現在のレイアウトを保存...」で名前を付けて保存
- メニューバーのアイコン →「レイアウト」→ 保存済みプリセット名をクリックで読み込み
- 「新しいレイアウトを作成...」でデフォルト画像1体のプリセットを作成
- 「上書き保存」で既存プリセットを現在の配置で更新
- 「削除」で不要なプリセットを削除

> シーンごと（仕事中・配信中・リラックスなど）にレイアウトを保存しておくと、気分に合わせて簡単に切り替えられます。
...
- ウィンドウID（#N）
- ウィンドウの重なり順
- 切り取り状態（クロップ領域・回転・傾き補正・パースペクティブ補正・反転）
...
| レイアウト | 保存済みプリセットの読み込み・保存・上書き・新規作成・削除 |
| 一括リセット | サブメニューで全ウィンドウの回転リセット・透明度リセット |
| すべて閉じる | すべてのウィンドウを閉じる |
| 設定 | サブメニューでログイン時に起動・デフォルト画像の変更/リセット・言語切り替え |
| 使い方ガイドを表示 | オンボーディングガイドを再表示 |
| 終了（`q`） | アプリケーションを終了 |
...
└── layouts/                      # レイアウトプリセット（JSON形式、プリセットごとに1ファイル）
```
