...
- **フローティングメニュー** - ダブルクリックでフローティングメニューを表示。切り取り・反転・調整・背景削除・表示をリセット・閉じるにワンクリックアクセス
- **画像の切り取り（トリミング）** - ドラッグハンドルで表示領域を非破壊的に調整。元の画像を保持したまま表示範囲を自由に変更
...
- フローティングメニューの「切り取り」、または右クリック →「その他」→「切り取り」で切り取りモードに入ります
- 四隅と辺のハンドルをドラッグして表示領域を調整します
- 「確定」で切り取りを適用、「リセット」で切り取り範囲を元に戻す、「キャンセル」で切り取りモードを終了します
- ESC キーでも切り取りモードから戻れます
