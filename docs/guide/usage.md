...
flowchart TD
    ...
    WIN_ACTIONS --> GHOST_ALPHA_W["ゴースト透明度"]
    ...
    SETTINGS --> GHOST_ALPHA["ゴーストモードの透明度"]
...
- ステータスバーメニュー →「設定」→「ゴーストモードの透明度」でグローバルのデフォルト透明度を変更可能（10%〜90%）
- 「表示中」サブメニュー →「ゴースト透明度」で個別ウィンドウの透明度をカスタマイズ可能
...
> 透明な背景でキャラクターを表示するため、**透過処理された PNG 画像の使用を推奨**します。背景のある画像は「背景を除去」（macOS 14以降）でワンクリック切り抜きできます。
