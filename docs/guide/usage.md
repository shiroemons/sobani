...
管理パネルはサイドバーで以下の5つのタブを切り替えます。ログタブと設定タブはサイドバー下部に固定表示されます。
...
| ログ | doc.text.magnifyingglass | 画面位置の診断ログの表示・フィルター・エクスポート |
...
画面位置の診断ログを表示します。スリープ復帰やモニター切断時のウィンドウ位置復元に問題がある場合、このログを有効にして動作を記録・確認できます。

**ツールバー**

| ボタン | 動作 |
|--------|------|
| リロード | ディスク上のログファイルを再読み込み |
| クリア | 全ログエントリを削除（確認ダイアログあり） |
| エクスポート | JSONL形式またはJSON形式でファイルに書き出し |

**フィルターバー**

| 項目 | 説明 |
|------|------|
| テキスト検索 | イベント名・ウィンドウID・コンテキスト値で絞り込み |
| イベント種別 | ドロップダウンで特定のイベント種別のみ表示 |
| ログ記録トグル | ログの記録を有効/無効に切り替え |

**ログテーブル**

| 列 | 内容 |
|----|------|
| Time | イベントの発生時刻（同日: HH:mm:ss、他日: MM/dd HH:mm:ss） |
| Event | イベント種別（カラーバッジで色分け表示） |
| Window | 対象ウィンドウID |
| Details | スクリーン数・コンテキスト情報のサマリー |

テーブルのヘッダーをクリックするとソート順を切り替えられます。行をクリックすると詳細パネルが表示されます。

**詳細パネル**

エントリを選択すると、以下の情報を含む詳細パネルが下部に表示されます:

- ミニマップ（スクリーン配置とウィンドウ位置の縮小表示）
- スクリーン情報テーブル（displayID、座標、サイズ、メインフラグ）
- ウィンドウ情報テーブル（フレーム座標、画像サイズ、displayID）
- コンテキスト情報（キー=値のグリッド表示）

**記録される情報と記録されない情報**

| 記録される | 記録されない |
|-----------|-------------|
| モニタ数・サイズ・配置 | 画像ファイル名・パス |
| ウィンドウ位置・サイズ | 画像の内容・サムネイル |
| Sleep/Wake/スクリーン変更イベント | 個人識別情報 |
| 座標計算（復元時） | |
