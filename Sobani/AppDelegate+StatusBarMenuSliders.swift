...
extension AppDelegate {
    ...
    func buildPerWindowGhostAlphaSliderItem(for imageWindow: ImageWindow) -> NSMenuItem {
        ...
        let checkbox = NSButton(
            checkboxWithTitle: "",
            target: self,
            action: #selector(togglePerWindowGhostAlphaCustom(_:))
        )
        checkbox.frame = NSRect(
            x: checkboxX,
            y: (containerHeight - checkboxSize) / 2,
            width: checkboxSize,
            height: checkboxSize
        )
        ...
        let sliderWidth = containerWidth - sliderX
            - AppConstants.ghostAlphaSliderPercentWidth
            - AppConstants.ghostAlphaSliderTrailingMargin
        let slider = NSSlider(
        ...
        slider.frame = NSRect(
            x: sliderX,
            y: (containerHeight - sliderHeight) / 2,
            width: sliderWidth,
            height: sliderHeight
        )
        ...
        let percentLabel = makePercentLabel(
            alpha: currentAlpha,
    ...
    private func makeSliderContainerBase(
        iconSymbol: String,
        labelText: String
    ) -> SliderContainerResult {
        let containerWidth = AppConstants.ghostAlphaSliderContainerWidth
        let containerHeight = AppConstants.ghostAlphaSliderContainerHeight
        let container = NSView(
            frame: NSRect(x: 0, y: 0, width: containerWidth, height: containerHeight)
        ...
        return SliderContainerResult(
            container: container,
            iconView: iconView,
            label: label,
            labelMaxX: label.frame.maxX
        )
    }
    ...
    func buildPerWindowOpacitySliderItem(for imageWindow: ImageWindow) -> NSMenuItem {
        ...
        let result = makeSliderContainerBase(
            iconSymbol: AppConstants.opacitySymbol,
            labelText: L("adjust.opacity")
        )
        ...
        container.addSubview(makePercentLabel(
            alpha: opacity,
    ...
    func buildGhostAlphaSliderItem() -> NSMenuItem {
        ...
        let result = makeSliderContainerBase(
            iconSymbol: AppConstants.ghostModeSymbol,
            labelText: L("ghost.alpha_setting")
        )
        let container = result.container
        let labelMaxX = result.labelMaxX
        let sliderX = labelMaxX + AppConstants.ghostAlphaSliderTrailingMargin
        container.addSubview(makeSlider(
            value: GhostModeSettings.globalAlpha,
        ...
        container.addSubview(makePercentLabel(
            alpha: GhostModeSettings.globalAlpha,
            containerWidth: AppConstants.ghostAlphaSliderContainerWidth,
