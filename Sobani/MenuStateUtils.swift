...
enum MenuStateUtils {
    ...
    static func hasRotation(angles: [CGFloat],
                            tolerance: CGFloat = AppConstants.floatingPointTolerance) -> Bool {
        angles.contains { abs($0) > tolerance }
    }

    /// いずれかの不透明度がデフォルト(1.0)から tolerance を超えて異なるか判定（一括不透明度リセット有効判定）
    static func hasNonDefaultOpacity(opacities: [CGFloat],
                                     tolerance: CGFloat = AppConstants.floatingPointTolerance) -> Bool {
        opacities.contains { abs($0 - 1.0) > tolerance }
    }

    /// 個別ウィンドウ回転リセット有効判定
    static func isRotationResetEnabled(angle: CGFloat,
                                       tolerance: CGFloat = AppConstants.floatingPointTolerance) -> Bool {
        abs(angle) > tolerance
    }

    /// 個別ウィンドウ不透明度リセット有効判定
    static func isOpacityResetEnabled(opacity: CGFloat,
                                      tolerance: CGFloat = AppConstants.floatingPointTolerance) -> Bool {
        abs(opacity - 1.0) > tolerance
    }

    /// 一括リセット親メニュー有効判定（いずれかがリセット可能ならtrue）
    static func isBulkResetEnabled(hasRotation: Bool, hasOpacity: Bool,
                                   hasGhost: Bool = false) -> Bool {
